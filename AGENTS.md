...
- Proxy environment clearing: yfinance searches are retried with proxy environment variables (`HTTP_PROXY`, `HTTPS_PROXY`, etc.) temporarily cleared to avoid corporate proxy issues. Implemented in `src/api.py` via `_temporarily_clear_proxy_env`.
...
| `src/ratio_analyzer.py` | Computes 40+ financial ratios across liquidity, solvency, profitability, and efficiency categories. |
...
| `src/services/_utils.py` | Shared helpers: `json_safe()` (`NaN`/`inf` to `None`), `safe_number()`, CJK detection. |
...
- Python 3.12+ is required and enforced in `pyproject.toml`.
