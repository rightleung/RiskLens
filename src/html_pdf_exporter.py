...
LANG = {
    'en': {
        ...
        'methodology_altman_note': 'Altman Z = 1.2 x working capital/assets + 1.4 x retained earnings/assets + 3.3 x EBIT/assets + 0.6 x equity market value/liabilities + 1.0 x sales/assets. It is a screening signal, not a default probability or agency rating; the market-value term can materially lift results for large public companies.',
        'methodology_zone_note': 'Zone bands: Z >= 2.99 Safe, 1.81 <= Z < 2.99 Grey, and Z < 1.81 Distress. These cutoffs support consistent triage, but they are not sector-neutral lending limits and should be read with trends, peers, and data quality.',
        'methodology_rating_note': 'Implied rating maps the Z-Score to an internal AAA-to-D display scale for relative ordering. It is not issued, endorsed, or equivalent to an S&P or other agency rating; missing historical market value may require an available-period proxy.',
    ...
    'zh-CN': {
        ...
        'methodology_altman_note': 'Altman Z = 1.2×营运资本/总资产 + 1.4×留存收益/总资产 + 3.3×EBIT/总资产 + 0.6×股权市值/总负债 + 1.0×营业收入/总资产。该分数用于财务困境筛查，不是违约概率或评级机构评级；对大型上市公司，市值项可能显著抬高结果。',
        'methodology_zone_note': '风险区间：Z ≥ 2.99 为 Safe，1.81 ≤ Z < 2.99 为 Grey，Z < 1.81 为 Distress。边界用于统一分层筛查，不是跨行业通用的授信限额，应结合趋势、同业和数据质量判断。',
        'methodology_rating_note': '隐含评级把 Z 分数映射为内部 AAA 至 D 展示尺度，用于相对风险排序；不代表 S&P 或其他评级机构出具、认可或等价的评级。历史市值缺失时，系统可能使用可取得期间的市值代理。',
    ...
    'zh-TW': {
        ...
        'methodology_altman_note': 'Altman Z = 1.2×營運資本/總資產 + 1.4×保留盈餘/總資產 + 3.3×EBIT/總資產 + 0.6×股權市值/總負債 + 1.0×營業收入/總資產。此分數用於財務困境篩查，不是違約機率或評級機構評等；對大型上市公司，市值項可能顯著推高結果。',
        'methodology_zone_note': '風險區間：Z ≥ 2.99 為 Safe，1.81 ≤ Z < 2.99 為 Grey，Z < 1.81 為 Distress。邊界用於一致分層篩查，不是跨行業通用的授信門檻，應結合趨勢、同業與資料品質判讀。',
        'methodology_rating_note': '隱含評等將 Z 分數映射為內部 AAA 至 D 顯示尺度，用於相對風險排序；不代表 S&P 或其他評級機構出具、認可或等價的評等。歷史市值缺失時，系統可能使用可取得期間的市值代理。',
    ...
    'ja': {
        ...
        'methodology_altman_note': 'Altman Z = 1.2 x 運転資本/総資産 + 1.4 x 利益剰余金/総資産 + 3.3 x EBIT/総資産 + 0.6 x 株式時価総額/負債 + 1.0 x 売上高/総資産。財務困難のスクリーニング指標であり、デフォルト確率や格付機関の格付けではありません。大型上場企業では時価総額項目がスコアを大きく押し上げることがあります。',
        'methodology_zone_note': 'ゾーン: Z >= 2.99 は Safe、1.81 <= Z < 2.99 は Grey、Z < 1.81 は Distress。区分は一貫した初期トリアージ用で、業種横断の融資限度ではありません。トレンド、同業比較、データ品質と併せて判断してください。',
        'methodology_rating_note': '推定格付けは Z スコアを内部表示用の AAA から D の尺度に変換し、相対的なリスク順位を示します。S&P などの格付機関が発行・承認した格付けではありません。過去の時価総額が欠落する場合、取得可能な期間の値を代理として使うことがあります。',
...
def _covenant_description(lang: str, metric: str) -> str:
    ...
    descriptions = {
        'en': {
            'debtebitda': 'Total debt divided by EBITDA. Shows the multiple of current operating earnings needed to cover gross debt before interest, taxes, capital expenditure, and working-capital changes. Lower is generally stronger; compare with sector norms and net debt.',
            'interestcoverage': 'EBIT divided by interest expense. Shows the operating-earnings cushion available to pay interest. Higher is stronger; a low or negative value signals limited coverage and does not capture principal repayment or refinancing risk.',
            'fcfdebt': 'Free cash flow divided by total debt. Approximates the share of debt that could be repaid from one period of cash generation after operating needs and capital expenditure. Higher is stronger, but working-capital swings and one-off cash items can distort it.',
            'currentratio': 'Current assets divided by current liabilities. Tests whether short-term assets cover obligations due within roughly one year. A ratio above one is not automatically safe because asset liquidity, timing, and committed facilities also matter.',
        },
        'zh-CN': {
            'debtebitda': '总债务 ÷ EBITDA，表示按当前经营收益覆盖总债务所需的倍数；未考虑利息、税项、资本开支和营运资金变动。通常越低越稳健，应结合行业水平及净债务判断。',
            'interestcoverage': 'EBIT ÷ 利息费用，表示经营收益可覆盖利息支出的缓冲倍数；通常越高越好。低值或负值意味着利息覆盖有限，且不包含本金偿还与再融资风险。',
            'fcfdebt': '自由现金流 ÷ 总债务，粗略估计一个期间的现金创造在满足经营需要和资本开支后可偿还的债务比例；通常越高越好，但会受到营运资金波动和一次性现金项目影响。',
            'currentratio': '流动资产 ÷ 流动负债，用于检验短期资产是否覆盖约一年内到期的负债；高于 1.0 并不自动代表安全，还要看资产变现能力、到期时点及已承诺融资额度。',
        },
        'zh-TW': {
            'debtebitda': '總債務 ÷ EBITDA，表示按目前營運收益覆蓋總債務所需的倍數；未考慮利息、稅項、資本支出與營運資金變動。通常越低越穩健，應結合行業水準及淨債務判斷。',
            'interestcoverage': 'EBIT ÷ 利息費用，表示營運收益可覆蓋利息支出的緩衝倍數；通常越高越好。低值或負值代表利息覆蓋有限，且不包含本金償還與再融資風險。',
            'fcfdebt': '自由現金流 ÷ 總債務，粗略估計一個期間的現金創造在滿足營運需求與資本支出後可償還的債務比例；通常越高越好，但會受到營運資金波動與一次性現金項目影響。',
            'currentratio': '流動資產 ÷ 流動負債，用於檢驗短期資產是否覆蓋約一年內到期的負債；高於 1.0 並不自動代表安全，還要看資產變現能力、到期時點及已承諾融資額度。',
        },
        'ja': {
            'debtebitda': '総有利子負債 ÷ EBITDA。利息・税金・設備投資・運転資本の変動を考慮する前の営業利益で、総負債を何倍でカバーするかを示します。一般に低いほど強く、業種水準と純有利子負債も確認します。',
            'interestcoverage': 'EBIT ÷ 支払利息。営業利益で利息を支払う余力を示します。一般に高いほど強く、低値またはマイナスは余力の乏しさを示しますが、元本返済と借換リスクは含みません。',
            'fcfdebt': 'フリーキャッシュフロー ÷ 総負債。営業必要資金と設備投資の後に、1期間のキャッシュ創出で返済できる負債の割合を近似します。高いほど強い一方、運転資本の変動や一時的な現金項目の影響を受けます。',
            'currentratio': '流動資産 ÷ 流動負債。おおむね1年以内に期限を迎える負債を短期資産でカバーできるかを確認します。1.0超でも安全とは限らず、資産の換金性、期限、コミット済み融資枠も確認が必要です。',
        },
    }
